...
describe('seedPersonaMemory (persona ตอน setup → durable memory ขั้นที่ 9)', () => {
  ...
  it('ข้ามค่าว่าง (blank = ไม่เขียน) แต่เก็บชื่อที่พิมพ์จริงแม้เท่ากับ default เดิม "Owner"', async () => {
    // raw blank/whitespace → not seeded (the wizard now passes '' on skip, no longer coerces to 'Owner')
    expect(await seedPersonaMemory({ ownerName: '', aiName: '   ' })).toBe(0);
    expect(activeFacts(await loadStore())).toHaveLength(0);
    // a name the user actually typed is kept even if it equals the legacy placeholder — the old
    // default-equality gate used to drop it (that was the "setup ลืมชื่อ" bug)
    expect(await seedPersonaMemory({ ownerName: 'Owner' })).toBe(1);
  });

  it('idempotent — เรียกซ้ำไม่เพิ่ม fact ใหม่', async () => {
    const input = { ownerName: 'เอก', aiName: 'มะนาว' };
  ...
  it('ปิด persistence → ไม่เขียน (คืน 0)', async () => {
    vi.stubEnv('SANOOK_DISABLE_PERSISTENCE', '1');
    expect(await seedPersonaMemory({ ownerName: 'x' })).toBe(0);
  });
});
