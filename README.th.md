...
- project `.sanook/config.json` ที่ยังไม่ trusted ตั้งค่าทั่วไปได้ แต่ค่าที่กระทบ state/safety ของเครื่อง (`permissionMode=auto`, `budgetUsd`, `pricing`, `brainPath`, `brainTranscript`, `autoMaintain`) จะถูก ignore จนกว่าจะ trust project
